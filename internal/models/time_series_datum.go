 ...
 *  return an error.
 */
func (dal *DataAccessLayer) FindTimeSeriesDatumByEmail(email string) (*TimeSeriesDatum, error) {
    user := TimeSeriesDatum{} // The struct which will be populated from the database.
 ...
 */
func (dal *DataAccessLayer) CreateTimeSeriesDatum(email string, firstName string, lastName string, password string, tenantId int64, tenantSchema string, roleId int64) (*TimeSeriesDatum, error) {
    ...
    return dal.FindTimeSeriesDatumByEmail(email)
}
